import {
  Create_User_AnyOtherAssets,
  Delete_User_AnyOtherAssets,
  get_User_AnyOtherAssets,
  Update_User_AnyOtherAssets,
} from '@/scripts/api'
...
export default function OtherAssets({
  ...
  const [AnyOtherAssetsType, setAnyOtherAssetsType] = useState()
  const [AnyOtherAssets, setAnyOtherAssets] = useState()
  ...
  const onFinish = async (values) => {
    ...
    if (selectedItem?.Id) {
      let res = await putData(
        Update_User_AnyOtherAssets + selectedItem?.Id,
        data
      )
      ...
      let res = await postData(Create_User_AnyOtherAssets, data)
  ...
  const getAnyOtherAssets = async () => {
    let res = await getData(get_User_AnyOtherAssets)

    if (res) {
      setAnyOtherAssets(res?.data || [])
    }
  }
  ...
  return (
    <div className='bg-white pb-6 px-6'>
      <h3 className='text-xl font-semibold'>OTHER ASSETS </h3>
      <ConfigProvider
        theme={{
          token: {
            colorPrimary: '#126A25',
