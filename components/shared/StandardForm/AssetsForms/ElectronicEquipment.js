import {
  Create_User_ElectronicEquipments,
  Delete_User_ElectronicEquipments,
  Get_User_ElectronicEquipments,
  Update_User_ElectronicEquipments,
} from '@/scripts/api'
...
export default function ElectronicEquipment({
  ...
  const [ElectronicEquipmentsType, setElectronicEquipmentsType] = useState()
  const [ElectronicEquipments, setElectronicEquipments] = useState()
  ...
  const onFinish = async (values) => {
    ...
    if (selectedItem?.Id) {
      let res = await putData(
        Update_User_ElectronicEquipments + selectedItem?.Id,
        data
      )
      ...
      let res = await postData(Create_User_ElectronicEquipments, data)
  ...
  const getElectronicEquipments = async () => {
    let res = await getData(Get_User_ElectronicEquipments)

    if (res) {
      setElectronicEquipments(res?.data || [])
    }
  }
  ...
  return (
    <div className='bg-white pb-6 px-6'>
      <h3 className='text-xl font-semibold'>ELECTRONIC EQUIPMENT </h3>
      <ConfigProvider
        theme={{
          token: {
            colorPrimary: '#126A25',
      ...
      >
        <div className='mt-5'>
          <Table
            columns={columns}
            dataSource={ElectronicEquipments}
            pagination={false}
          />
