import {
  Create_liability_OtherLiabilities,
  Delete_liability_OtherLiabilities,
  Get_liability_OtherLiabilities,
  Update_liability_OtherLiabilities,
} from '@/scripts/api'
...
export default function OtherLiabilities({
  ...
  const [OtherLiabilitiesType, setOtherLiabilitiesType] = useState()
  ...
  const onFinish = async (values) => {
    ...
    if (selectedItem?.Id) {
      let res = await putData(
        Update_liability_OtherLiabilities + selectedItem?.Id,
        data
      )
      ...
      let res = await postData(Create_liability_OtherLiabilities, data)
  ...
  const getOtherLiabilities = async () => {
    let res = await getData(Get_liability_OtherLiabilities)
  ...
  return (
    <div className='bg-white pb-6 px-6'>
      <h3 className='text-xl font-semibold'>OTHER LIABILITIES</h3>
      <ConfigProvider
        theme={{
          token: {
            colorPrimary: '#126A25',
