import {
  Create_liability_UnsecuredLoans,
  Delete_liability_UnsecuredLoans,
  Get_liability_UnsecuredLoans,
  Update_liability_UnsecuredLoans,
} from '@/scripts/api'
...
export default function UnsecuredLoans({
  ...
  const [UnsecuredLoansType, setUnsecuredLoansType] = useState()
  ...
  const onFinish = async (values) => {
    ...
    if (selectedItem?.Id) {
      let res = await putData(
        Update_liability_UnsecuredLoans + selectedItem?.Id,
        data
      )
      ...
      let res = await postData(Create_liability_UnsecuredLoans, data)
  ...
  const getUnsecuredLoans = async () => {
    let res = await getData(Get_liability_UnsecuredLoans)

    if (res) {
      setUnsecuredLoans(res?.data || [])
    }
  }
  ...
  return (
    <div className='bg-white pb-6 px-6'>
      <h3 className='text-xl font-semibold'>UNSECURED LOANS </h3>
      <ConfigProvider
        theme={{
          token: {
            colorPrimary: '#126A25',
