import {
  Create_Expense_TelephoneBills,
  Delete_Expense_TelephoneBills,
  Get_Expense_TelephoneBills,
  Update_Expense_TelephoneBills,
} from '@/scripts/api'
...
export default function Telephone({
  ...
  const onFinish = async (values) => {
    ...
    if (selectedItem?.Id) {
      let res = await putData(
        Update_Expense_TelephoneBills + selectedItem?.Id,
        data
      )
  ...
  const getTelephoneBills = async () => {
    let res = await getData(Get_Expense_TelephoneBills)

    if (res) {
      setTelephoneBills(res?.data || [])
    }
  }
  ...
  const deleteItem = (Id) => {
    confirm({
      ...
      async onOk() {
        let res = await deleteData(Delete_Expense_TelephoneBills + Id)
        if (res) {
          getTelephoneBills()
