import {
  Create_Expense_TaxSurchargeOthers,
  Delete_Expense_TaxSurchargeOthers,
  Get_Expense_TaxSurchargeOthers,
  Update_Expense_TaxSurchargeOthers,
} from '@/scripts/api'
...
export default function TaxPaidSurchargeandOther({
  setActiveTab,
  nextActiveTab,
  setProsCurrent,
  backActiveTab,
  ...
  const onFinish = async (values) => {
    ...
    if (selectedItem?.Id) {
      let res = await putData(
        Update_Expense_TaxSurchargeOthers + selectedItem?.Id,
        data
      )
      ...
      let res = await postData(Create_Expense_TaxSurchargeOthers, data)
  ...
  const getTaxSurchargeOthers = async () => {
    let res = await getData(Get_Expense_TaxSurchargeOthers)

    if (res) {
      setTaxSurchargeOthers(res?.data || [])
    }
  }
  ...
  useEffect(() => {
    if (comment && typeof comment === 'string') {
      form.setFieldsValue({SurchargeOtherComment: comment})
    }
  }, [comment])
  ...
  return (
    <div className='bg-white pb-6 px-6'>
      <h3 className='text-xl font-semibold'>
        TAX PAID LAST YEAR, SURCHARGE AND OTHERS EXPENSES{' '}
      </h3>
      <ConfigProvider
        theme={{
          token: {
