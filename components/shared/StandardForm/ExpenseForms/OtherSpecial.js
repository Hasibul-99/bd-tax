import {
  Create_Expense_OtherSpecials,
  Delete_Expense_OtherSpecials,
  Get_Expense_OtherSpecials,
  Update_Expense_OtherSpecials,
} from '@/scripts/api'
...
export default function OtherSpecial({
  ...
  const onFinish = async (values) => {
    ...
    if (selectedItem?.Id) {
      let res = await putData(
        Update_Expense_OtherSpecials + selectedItem?.Id,
        data
      )
      ...
      let res = await postData(Create_Expense_OtherSpecials, data)
  ...
  const getOtherSpecials = async () => {
    let res = await getData(Get_Expense_OtherSpecials)

    if (res) {
      setOtherSpecials(res?.data || [])
    }
  }
  ...
  const deleteItem = (Id) => {
    confirm({
      ...
      async onOk() {
        let res = await deleteData(Delete_Expense_OtherSpecials + Id)
        if (res) {
          getOtherSpecials()
