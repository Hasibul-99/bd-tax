import {
  Create_Expense_OtherTransportations,
  Delete_Expense_OtherTransportations,
  Get_Expense_OtherTransportations,
  Update_Expense_OtherTransportations,
} from '@/scripts/api'
...
export default function OtherTransportation({
  ...
  const onFinish = async (values) => {
    ...
    if (selectedItem?.Id) {
      let res = await putData(
        Update_Expense_OtherTransportations + selectedItem?.Id,
        data
      )
      ...
      let res = await postData(Create_Expense_OtherTransportations, data)
  ...
  const getOtherTransportations = async () => {
    let res = await getData(Get_Expense_OtherTransportations)

    if (res) {
      setOtherTransportations(res?.data || [])
    }
  }
  ...
  const deleteItem = (Id) => {
    confirm({
      ...
      async onOk() {
        let res = await deleteData(Delete_Expense_OtherTransportations + Id)
        if (res) {
          getOtherTransportations()
  ...
  useEffect(() => {
    if (comment && typeof comment === 'string') {
      form.setFieldsValue({OtherTransportComment: comment})
    }
  }, [comment])
    ...
    <div className='bg-white pb-6 px-6'>
      ...
      >
        <div className='mt-5'>
          <Table
            columns={columns}
            dataSource={OtherTransportations}
            pagination={false}
          />
