import {
  Create_Expense_ElectricityBills,
  Delete_Expense_ElectricityBills,
  Get_Expense_ElectricityBills,
  Update_Expense_ElectricityBills,
} from '@/scripts/api'
...
export default function Electricity({
  ...
  const onFinish = async (values) => {
    ...
    if (selectedItem?.Id) {
      let res = await putData(
        Update_Expense_ElectricityBills + selectedItem?.Id,
        data
      )
      ...
      let res = await postData(Create_Expense_ElectricityBills, data)
  ...
  const getElectricityBills = async () => {
    let res = await getData(Get_Expense_ElectricityBills)

    if (res) {
      setElectricityBills(res?.data || [])
    }
  }
  ...
  const deleteItem = (Id) => {
    confirm({
      ...
      async onOk() {
        let res = await deleteData(Delete_Expense_ElectricityBills + Id)
        if (res) {
          getElectricityBills()
