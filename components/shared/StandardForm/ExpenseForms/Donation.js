import {
  Create_Expense_Donations,
  Delete_Expense_Donations,
  Get_Expense_Donations,
  Update_Expense_Donations,
} from '@/scripts/api'
...
export default function Donation({
  ...
  const onFinish = async (values) => {
    ...
    if (selectedItem?.Id) {
      let res = await putData(Update_Expense_Donations + selectedItem?.Id, data)
  ...
  const getDonations = async () => {
    let res = await getData(Get_Expense_Donations)
  ...
  const deleteItem = (Id) => {
    confirm({
      ...
      async onOk() {
        let res = await deleteData(Delete_Expense_Donations + Id)
        if (res) {
          getDonations()
  ...
  return (
    <div className='bg-white pb-6 px-6'>
      ...
      >
        <div className='mt-5'>
          <Table columns={columns} dataSource={Donations} pagination={false} />
