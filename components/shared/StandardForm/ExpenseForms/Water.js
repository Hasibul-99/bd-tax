import {
  Create_Expense_WaterBills,
  Delete_Expense_WaterBills,
  Get_Expense_WaterBills,
  Update_Expense_WaterBills,
} from '@/scripts/api'
...
export default function Water({
  ...
  const onFinish = async (values) => {
    ...
    if (selectedItem?.Id) {
      let res = await putData(
        Update_Expense_WaterBills + selectedItem?.Id,
        data
      )
      ...
      let res = await postData(Create_Expense_WaterBills, data)
  ...
  const getWaterBills = async () => {
    let res = await getData(Get_Expense_WaterBills)
  ...
  const deleteItem = (Id) => {
    confirm({
      ...
      async onOk() {
        let res = await deleteData(Delete_Expense_WaterBills + Id)
        if (res) {
          getWaterBills()
