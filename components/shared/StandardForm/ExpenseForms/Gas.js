import {
  Create_Expense_GasBills,
  Delete_Expense_GasBills,
  Get_Expense_GasBills,
  Update_Expense_GasBills,
} from '@/scripts/api'
...
export default function Gas({
  ...
  const onFinish = async (values) => {
    ...
    if (selectedItem?.Id) {
      let res = await putData(Update_Expense_GasBills + selectedItem?.Id, data)
      ...
      let res = await postData(Create_Expense_GasBills, data)
  ...
  const getGasBills = async () => {
    let res = await getData(Get_Expense_GasBills)

    if (res) {
      setGasBills(res?.data || [])
    }
  }
  ...
  const deleteItem = (Id) => {
    confirm({
      ...
      async onOk() {
        let res = await deleteData(Delete_Expense_GasBills + Id)
        if (res) {
          getGasBills()
  ...
  return (
    <div className='bg-white pb-6 px-6'>
      ...
      >
        <div className='mt-5'>
          <Table columns={columns} dataSource={GasBills} pagination={false} />
