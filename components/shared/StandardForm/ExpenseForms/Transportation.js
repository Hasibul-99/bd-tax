import {
  Create_Expense_Transportations,
  Delete_Expense_Transportations,
  Get_Expense_Transportations,
  Update_Expense_Transportations,
} from '@/scripts/api'
...
export default function Transportation({
  ...
  const onFinish = async (values) => {
    ...
    if (selectedItem?.Id) {
      let res = await putData(
        Update_Expense_Transportations + selectedItem?.Id,
        data
      )
  ...
  const getTransportations = async () => {
    let res = await getData(Get_Expense_Transportations)
  ...
  const deleteItem = (Id) => {
    confirm({
      ...
      async onOk() {
        let res = await deleteData(Delete_Expense_Transportations + Id)
        if (res) {
          getTransportations()
