import {
  Create_Expense_ChildrenEducations,
  Delete_Expense_ChildrenEducations,
  Get_Expense_ChildrenEducations,
  Update_Expense_ChildrenEducations,
} from '@/scripts/api'
...
export default function ChildrenEducation({
  ...
  const onFinish = async (values) => {
    ...
    if (selectedItem?.Id) {
      let res = await putData(
        Update_Expense_ChildrenEducations + selectedItem?.Id,
        data
      )
      ...
      let res = await postData(Create_Expense_ChildrenEducations, data)
  ...
  const getChildrenEducations = async () => {
    let res = await getData(Get_Expense_ChildrenEducations)

    if (res) {
      setChildrenEducations(res?.data || [])
    }
  }
  ...
  const deleteItem = (Id) => {
    confirm({
      ...
      async onOk() {
        let res = await deleteData(Delete_Expense_ChildrenEducations + Id)
        if (res) {
          getChildrenEducations()
